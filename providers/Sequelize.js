...
module.exports = class SequelizeProvider extends SettingProvider {
    ...
    constructor(sequelize) {
        ...
        /**
         * Client that the provider is for (set once the client is ready, after using
         * {@link CommandoClient#setProvider})
         ...
         * @private
         */
        this.model = this.sequelize.define('guilds', {
            guild: {
    ...
    async init(client) {
        ...
        for (const row of rows) {
            ...
            try {
                ...
                client.emit('warn',
                            `Sequelize Provider couldn't parse the settings stored for guild ${row.dataValues.guild}.`);
                continue;
            }
        ...
        this.listeners
            ...
            .set('commandStatusChange',
                 (guild, command, enabled) => this.set(guild, `cmd-${command.name}`, enabled))
            .set('groupStatusChange',
                 (guild, group, enabled) => this.set(guild, `grp-${group.name}`, enabled))
    ...
    setupGuild(guild, settings) {
        if (typeof guild !== 'string') {
            throw new TypeError(
                'The guild must be a guild ID or "global".');
        }
        ...
        for (const command of this.client.registry.commands.values()) {
            this.setupGuildCommand(guild,
                                   command,
                                   settings);
        }
        for (const group of this.client.registry.groups.values()) {
            this.setupGuildGroup(guild, group,
                                 settings);
        }
    }
